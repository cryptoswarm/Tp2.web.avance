...
from src import db
from src.models.coordiante import CoordinateSchema
import hashlib
...
class InstallationAquatique(Base):
    ...
    propriete_installation = db.Column(
        db.String(255), nullable=False, default="UNKNOWN"
    )
    ...
    def __repr__(self):
        return (
            "<InstallationAquatique(installation_id='%d', \
                                       nom_installation='%s', \
                                       type_installation='%s',\
                                       adress='%s', \
