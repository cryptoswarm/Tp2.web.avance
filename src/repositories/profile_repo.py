...
from src.models.followed_arr import InspectedArr


def delete_profile(email, profile_id):
    followed_arr = InspectedArr.query.filter_by(profile_id=profile_id).delete()
    profile = Profile.query.filter_by(email=email).delete()
    db.session.commit()
...
def find_followed_arr_by_name(name: str) -> InspectedArr or None:
    return InspectedArr.query.filter_by(name=name).first()


def find_followed_by_profile_id(id: int):
    return InspectedArr.query.filter_by(profile_id=id).all()


def find_followed_arr_name_id(name: str, id: int) -> InspectedArr or None:
    return InspectedArr.query.filter(
        and_((InspectedArr.name == name), (InspectedArr.profile_id == id))
    ).first()
