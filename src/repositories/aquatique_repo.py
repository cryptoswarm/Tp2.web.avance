import logging
from src import db
from src.models.inst_aquatique import InstallationAquatique
...
def find_aqua_inst_names_arr_id(arr_id):
    return (
        InstallationAquatique.query.with_entities(
            InstallationAquatique.nom_installation, InstallationAquatique.id
        )
...
def find_aqua_installations(arr_id, aqua_name):
    response = InstallationAquatique.query.filter(
        and_(
