...
from src.models.installation import InstallationsSchema
...
pat_cond_schema = PatAndConditionSchema(
    many=True, exclude=("id", "arron_id", "conditions.id", "conditions.patinoire_id")
)
...
def get_installation_arr_name():
    ...
    if arr_name is not None:
        ...
        if inst_names is None:
            return jsonify({"message": "No installations has been found"}), 404
